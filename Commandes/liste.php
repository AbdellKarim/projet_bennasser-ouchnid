...
if (empty($message_erreur)) {
    ...
    if ($resultat) {
        if (mysqli_num_rows($resultat) == 0) {
            $message .= "Aucune commande trouvée<br>\n";
        } else {
            // Construction du tableau
            $tableau_commandes = "<table>\n<thead>\n<tr>
                                    <th>N° Commande</th>
                                    <th>N° Client</th> 
                                    <th>Date</th>
                                    <th>Actions</th>
            ...
            while ($ligne = mysqli_fetch_assoc($resultat)) {
                $tableau_commandes .= "<tr>
                                        <td>" . htmlspecialchars($ligne['NCom']) . "</td>
                                        <td>" . htmlspecialchars($ligne['NCli']) . "</td>
                                        <td>" . htmlspecialchars($ligne['DateCom']) . "</td>
                                        <td class='actions'>
                                            <a href='modifier.php?id=" . $ligne['NCom'] . "' class='edit'>Modifier</a>
                                            <a href='supprimer.php?id=" . $ligne['NCom'] . "' class='delete' onclick='return confirm(\"Voulez-vous vraiment supprimer cette commande ?\");'>Supprimer</a>
                                        </td>
                                    </tr>\n";
            }

            $tableau_commandes .= "</tbody>\n</table>\n";
        }
    } else {
